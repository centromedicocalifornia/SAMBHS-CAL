...
{
    public partial class frmMigrarProductoUtilidadesDescuentos2 : Form
    {
        DatahierarchyBL _objDatahierarchyBL = new DatahierarchyBL();
        public frmMigrarProductoUtilidadesDescuentos2()
        {
            InitializeComponent();
        }

        private void frmMigrarProductoUtilidadesDescuentos2_Load(object sender, EventArgs e)
        {
        ...
        private void ultraButton4_Click(object sender, EventArgs e)
        {
            ...
            foreach (UltraGridCell cell in grdData.Rows[0].Cells)
            {
                ...
                cboAlmacen.Items.Add(ColumnKey);
            }
        ...
        private void ultraButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal mon = 0;
                List<SAMBHS.Almacen.BL.ProductoBL.ProductoListaUtilidadDto> listaResult = new List<SAMBHS.Almacen.BL.ProductoBL.ProductoListaUtilidadDto>();
                if (rbCostos.Checked)
                {
                    listaResult = filas.Select(fila => new SAMBHS.Almacen.BL.ProductoBL.ProductoListaUtilidadDto
                    {
                ...
                else
                {
                    listaResult = filas.Select(fila => new SAMBHS.Almacen.BL.ProductoBL.ProductoListaUtilidadDto
                   {
                       Monto = decimal.TryParse(fila.Cells[cboMonto.Text].Value.ToString().Trim(), out mon) ? mon : 0,
                       CodProducto = fila.Cells[cboCodProd.Text].Value.ToString(),
                       IdAlmacen = int.Parse(fila.Cells[cboAlmacen.Text].Value.ToString()),
                       IdLista = int.Parse(cboListaPrecios.Value.ToString()),
                   })
                   .ToList();
                }
                ...
                if (!rbCostos.Checked)
                {
                   Mensaje = new ProductoBL().ExisteListaPreciosPorAlmacen(listaResult);
                }
                    ...
                    using (new PleaseWait(this.Location, "Por favor espere..."))
                    {
                        if (!rbCostos.Checked)
                        {
                            objProductoBl.ActualizarListaPreciosUtilidadMigracion(ref objOperationResult, idListaPrecio,
                                listaResult,
                                rbUtilidades.Checked
                                ? ProductoBL.ProductoListaTipo.Utilidad : rbPrecios.Checked ? ProductoBL.ProductoListaTipo.Precio :
