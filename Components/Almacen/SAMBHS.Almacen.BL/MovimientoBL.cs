...
namespace SAMBHS.Almacen.BL
{
    public class MovimientoBL
    {
        ...
        public string InsertarMovimiento(ref OperationResult pobjOperationResult, movimientoDto pobjDtoEntity, List<string> ClientSession, List<movimientodetalleDto> pTemp_Insertar)
        {
            ...
            try
            {
                using (var ts = TransactionUtils.CreateTransactionScope())
                {
                    ...
                    using (var dbContext = new SAMBHSEntitiesModelWin())
                    {
                        dbContext.CommandTimeout = 864000;
        ...
        public List<movimientoDto> ListarBusquedaMovimientosDetallado(ref OperationResult pobjOperationResult, string pstrSortExpression, string pstrFilterExpression, DateTime F_Ini, DateTime F_Fin, int pintTipoMovimiento, int Establecimiento)
        {
            try
            {
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    ...
                    var query = (from n in dbContext.movimiento
                                 ...
                                 join A1 in dbContext.datahierarchy on new { ao = n.i_GenerarGuia.Value, B = 90 } equals
                                     new { ao = A1.i_ItemId, B = A1.i_GroupId } into A1_join
                                 from A1 in A1_join.DefaultIfEmpty()

                                 join A2 in dbContext.movimientodetalle on n.v_IdMovimiento equals A2.v_IdMovimiento
                                 join A3 in dbContext.productodetalle on A2.v_IdProductoDetalle equals A3.v_IdProductoDetalle
                                 join A4 in dbContext.producto on A3.v_IdProducto equals A4.v_IdProducto

                                 join A5 in dbContext.datahierarchy on new { ao = A4.i_IdUnidadMedida.Value, B = 17 } equals
                                     new { ao = A5.i_ItemId, B = A5.i_GroupId } into A5_join
                                 from A5 in A5_join.DefaultIfEmpty()

                                 join A6 in dbContext.marca on new { ao = A4.v_IdMarca, eliminado = 0 } equals
                                     new { ao = A6.v_IdMarca, eliminado = A6.i_Eliminado.Value } into A6_join
                                 from A6 in A6_join.DefaultIfEmpty()
                                ...
                                && (n.i_IdEstablecimiento == Establecimiento || Establecimiento == -1)
                                 select new movimientoDto
                                 {
                                     ...
                                     Producto = A4.v_Descripcion,
                                     PrecioBaseU = A4.d_PrecioVenta,
                                     Cantidad = A2.d_Cantidad,
                                     PrecioEgresoU = A2.d_Precio,
                                     TotalMov = A2.d_Total,
                                     Glosa = n.v_Glosa,
                                     Unidad = A5.v_Value1,
                                     Marca = A6.v_Nombre
        ...
        public List<movimientodetalleDto> ObtenerDetalleKardexManguifajasFecha(ref OperationResult objOperationResult, int idAlmacen, string pstrProductoDetalle, int FormatoCant, DateTime FechaDesde, DateTime FechaHasta)
        {
            try
            {
                ...
                dbContext.CommandTimeout = 10000;
                ...
                //{
                var f = DateTime.Parse(FechaDesde.ToShortDateString() + " 00:00");
                //var FechaAnt = f.AddDays(-1);
                var FechaAnt = f.AddDays(0);
        ...
        private static void ActualizarCostoSegunUltimoMovimiento(ref OperationResult pobjOperationResult, movimientoDto pobjMovimientoEntrada, List<movimientodetalleDto> pobjMovimientoEntradaDetalle, bool eliminacion = false)
        {
            try
            {
                ...
                using (var ts = TransactionUtils.CreateTransactionScope())
                {
                    ...
                    using (var dbContext = new SAMBHSEntitiesModelWin())
                    {
                        //dbContext.CommandTimeout = 900000;
                        dbContext.CommandTimeout = 999999;
                        ...
                        if (pobjMovimientoEntrada != null && pobjMovimientoEntrada.t_Fecha != null)
                        {
                            ...
                            pobjMovimientoEntradaDetalle = pobjMovimientoEntradaDetalle.FindAll(p => p.d_Precio.Value > 0).ToList();
                            foreach (var movimientoDetalle in pobjMovimientoEntradaDetalle)
                            {
        ...
        //            var validacion = cnx.Query<ProductoStock>(query).FirstOrDefault();
