...
{
    public partial class frmIgualarUM : Form
    {
        DatahierarchyBL _objDatahierarchyBL = new DatahierarchyBL();
        UltraCombo ucUnidadMedida = new UltraCombo();
        List<string> UnidadesMedida = new List<string>();
        List<GridKeyValueDTO> DSComboUM = new List<GridKeyValueDTO>();
        public frmIgualarUM(List<string> UM)
        {
            UnidadesMedida = UM;
            InitializeComponent();
        }

        private void frmIgualarUM_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                columnUMdb.DataType = typeof(int);
                CargarCombos();
                
                foreach (string UM in UnidadesMedida)
                {
                ...
                if (grdData.Rows.Count() > 0)
                {
                    ...
                    {
                        var xl = DSComboUM.Where(p => p.Value1 == Fila.Cells["v_UM"].Value.ToString().Trim().ToUpper());
                        if (xl.Count() != 0)
                        {
        ...
        private void CargarCombos()
        {
            ...
            DSComboUM = _objDatahierarchyBL.GetDataHierarchiesForComboGrid(ref objOperationResult, 17, null);
            Utils.Windows.LoadUltraComboList(ucUnidadMedida, "Value1", "Id", DSComboUM, DropDownListAction.Select);
        }

        private void ultraGrid1_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
            e.Layout.Bands[0].Columns["i_UMdb"].EditorComponent = ucUnidadMedida;
        ...
        private void ultraButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    (System.Windows.Forms.Application.OpenForms["frmMigraciones"] as frmMigraciones).RecibirListaUM(ListaResultado);
                }
                this.Close();
            }
        }
