...
{
    public partial class frmReporteCostoUtilidadDetallado : Form
    {
        EstablecimientoBL objEstablecimientoBL = new EstablecimientoBL();
        DatahierarchyBL _objDatahierarchyBL = new DatahierarchyBL();
        CancellationTokenSource _cts = new CancellationTokenSource();
        public frmReporteCostoUtilidadDetallado(string N)
        {
            InitializeComponent();
        }
        ...
        {
            List<ReporteCostoUtilidadDetallado> ReporteFinal = new List<ReporteCostoUtilidadDetallado>();
            List<ReporteCostoUtilidadDetallado> ReporteFiltrado = new List<ReporteCostoUtilidadDetallado>(); 
            List<ReporteCostoUtilidadDetalladoDetalles> ReporteFinalDetalles = new List<ReporteCostoUtilidadDetalladoDetalles>();
            OperationResult objOperationResult = new OperationResult();
            OcultarMostrarBuscar(true);
            Cursor.Current = Cursors.WaitCursor;
            var rp = new Reportes.Ventas.crReporteVentasCostoUtilidadDetallado();
            var rp1 = new Reportes.Ventas.crReporteVentasCostoUtilidadDetalladoDetalles();
            Task.Factory.StartNew(() =>
            {
                ReporteFinal = new VentaBL().ReporteCostoUtilidadDetallado(ref objOperationResult, int.Parse ( nupMesInicio.Value.ToString()), int.Parse (nupMesFin.Value.ToString()), cboVendedor.Value.ToString(), int.Parse(cboAlmacen.Value.ToString()), txtPedidoInicio.Text.Trim(), txtPedidoFin.Text.Trim() ,int.Parse (cboMoneda.Value.ToString ()),Globals.ClientSession.i_Periodo.ToString ());
                if (objOperationResult.Success == 1)
                {
                    ReporteFinalDetalles = new VentaBL().ReporteCostoUtilidadDetalladoDetalles(ReporteFinal, int.Parse(cboMoneda.Value.ToString()), Globals.ClientSession.i_Periodo.ToString());
                }
            }, _cts.Token)
            .ContinueWith(t =>
            {
                ...
                if (objOperationResult.Success == 0)
                {
                    ...
                    {
                        UltraMessageBox.Show("Ocurrió un Error en Reporte Costo Utilidad sobre venta por Pedido( Detallado )", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                        UltraMessageBox.Show("Ocurrió un Error en Reporte Costo Utilidad sobre venta por Pedido( Detallado )", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                DataTable dt2 = SAMBHS.Common.Resource.Utils.ConvertToDatatable(ReporteFinalDetalles);
