...
namespace NetPdf
{
    public class TicketHospit
    {
        ...
        public static void CreateTicket_Hospi(string filePDF, OrganizationDto1 infoEmpresaPropietaria, List<SAMBHS.Common.BE.Custom.Medicina_Tickets> datos)
        {
            ...
            if (datos != null)
            {
                ...
                foreach (var item in datos)
                {
                    ...
                    if (item.Factor == 0)
                    {
                        cell = new PdfPCell(new Phrase(decimal.Round((item.Precio), 2).ToString(), fontColumnValue)) { HorizontalAlignment = PdfPCell.ALIGN_CENTER, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.BLACK, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                        cells.Add(cell);
                        cell = new PdfPCell(new Phrase(decimal.Round((item.Precio), 2).ToString(), fontColumnValue)) { HorizontalAlignment = PdfPCell.ALIGN_CENTER, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.BLACK, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                        cells.Add(cell);
                        suma += (decimal.Round((item.Precio), 2));
                        cell = new PdfPCell(new Phrase("-", fontColumnValue)) { Rowspan = tamañoTickets, HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.BLACK, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                        cells.Add(cell);
                    }
                    else
                    {
